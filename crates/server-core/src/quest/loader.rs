...
pub fn load_quest_registry(base: &WzNodeArc) -> HashMap<u32, QuestDef> {
    ...
    let info_node = match wz_cli::resolve_path(base, "Quest/QuestInfo.img") {
        Some(n) => n,
        None => return quests,
    };
    let check_node = wz_cli::resolve_path(base, "Quest/Check.img");
    let act_node = wz_cli::resolve_path(base, "Quest/Act.img");

    for (id_str, info_child) in wz_cli::get_children(&info_node) {
        ...
        let start_check = check_node
            .as_ref()
            .and_then(|n| wz_cli::resolve_path(n, &format!("{quest_id}/0")))
            .map(|n| parse_check_conditions(&n))
            .unwrap_or_default();

        let complete_check = check_node
            .as_ref()
            .and_then(|n| wz_cli::resolve_path(n, &format!("{quest_id}/1")))
            .map(|n| parse_check_conditions(&n))
            .unwrap_or_default();
        ...
        let start_script = check_node
            .as_ref()
            .and_then(|n| wz_cli::resolve_path(n, &format!("{quest_id}/0/startscript")))
            .and_then(|n| n.read().ok()?.try_as_string()?.get_string().ok())
            .map(|s| s.to_string());

        let end_script = check_node
            .as_ref()
            .and_then(|n| wz_cli::resolve_path(n, &format!("{quest_id}/1/endscript")))
            .and_then(|n| n.read().ok()?.try_as_string()?.get_string().ok())
            .map(|s| s.to_string());
...
fn parse_check_conditions(node: &WzNodeArc) -> CheckConditions {
    ...
    if let Some(job_node) = wz_cli::resolve_path(node, "job") {
        let jobs: Vec<u32> = wz_cli::get_children(&job_node)
            .iter()
    ...
    if let Some(quest_node) = wz_cli::resolve_path(node, "quest") {
        for (_, child) in wz_cli::get_children(&quest_node) {
    ...
    if let Some(item_node) = wz_cli::resolve_path(node, "item") {
        for (_, child) in wz_cli::get_children(&item_node) {
            let id = get_int_child(&child, "id").unwrap_or(0) as u32;
    ...
    if let Some(mob_node) = wz_cli::resolve_path(node, "mob") {
        for (_, child) in wz_cli::get_children(&mob_node) {
            let id = get_int_child(&child, "id").unwrap_or(0) as u32;
    ...
    if let Some(skill_node) = wz_cli::resolve_path(node, "skill") {
        for (_, child) in wz_cli::get_children(&skill_node) {
            let id = get_int_child(&child, "id").unwrap_or(0) as u32;
    ...
    conds.has_script = wz_cli::resolve_path(node, "startscript").is_some()
        || wz_cli::resolve_path(node, "endscript").is_some();
...
fn parse_quest_actions(node: &WzNodeArc) -> QuestActions {
    ...
    if let Some(skill_node) = wz_cli::resolve_path(node, "skill") {
        for (_, child) in wz_cli::get_children(&skill_node) {
            ...
            let job_whitelist = if let Some(job_node) = wz_cli::resolve_path(&child, "job") {
                wz_cli::get_children(&job_node)
                    .iter()
...
fn get_int_child(node: &WzNodeArc, name: &str) -> Option<i32> {
    let child = wz_cli::resolve_path(node, name)?;
