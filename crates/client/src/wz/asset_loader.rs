        ...
        for (layer_name, group_node) in fh_root.children() {
            ...
            for (_group_name, id_node) in group_node.children() {
                for (id_name, fh) in id_node.children() {
                    let x1: f32 = fh.at_path("x1").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
                    let y1: f32 = fh.at_path("y1").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
                    let x2: f32 = fh.at_path("x2").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
                    let y2: f32 = fh.at_path("y2").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
...
fn load_backgrounds(
    ...
    for (name, back_node) in children {
        ...
        let x: f32 = back_node.at_path("x").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
        let y: f32 = back_node.at_path("y").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
...
fn load_life(map: &crate::wz::Node) -> Vec<LifeSpawn> {
    ...
    for (_name, life_node) in life_root.children() {
        ...
        let x: f32 = life_node.at_path("x").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
        let y: f32 = life_node.at_path("y").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
...
fn load_portals(map: &crate::wz::Node) -> Vec<PortalData> {
    ...
    for (_name, portal_node) in portal_root.children() {
        ...
        let x: f32 = portal_node.at_path("x").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
        let y: f32 = portal_node.at_path("y").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
...
fn load_ladder_ropes(map: &crate::wz::Node) -> Vec<LadderRopeData> {
    ...
    for (_name, lr_node) in lr_root.children() {
        ...
        let y1: f32 = lr_node.at_path("y1").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
        let y2: f32 = lr_node.at_path("y2").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
...
fn load_seats(map: &crate::wz::Node) -> Vec<SeatData> {
    ...
    for (_name, seat_node) in seat_root.children() {
        let x: f32 = seat_node.at_path("x").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
        let y: f32 = seat_node.at_path("y").ok().and_then(|n| n.try_into().ok()).unwrap_or(0.0);
...
fn load_areas(map: &crate::wz::Node) -> Vec<AreaData> {
    ...
    for (_name, area_node) in area_root.children() {
        ...
        let y1: i32 = area_node.at_path("y1").ok().and_then(|n| n.try_into().ok()).unwrap_or(0);
        let x2: i32 = area_node.at_path("x2").ok().and_then(|n| n.try_into().ok()).unwrap_or(0);
        let y2: i32 = area_node.at_path("y2").ok().and_then(|n| n.try_into().ok()).unwrap_or(0);
        areas.push(AreaData { x1, y1, x2, y2 });
    }
...
fn load_minimap(
    ...
    let y: Option<i32> = mm_node.at_path("y").ok().and_then(|n| n.try_into().ok());
