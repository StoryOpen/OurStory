...
pub fn tick_npc_animations(
    ...
    for (mut animator, mut sprite, mut transform, npc_id) in &mut npc_query {
        ...
        sprite.image = frame.sprite.image_handle.clone();
        transform.translation.x = animator.base_x - frame.sprite.origin.x;
        transform.translation.y = animator.base_y - frame.sprite.origin.y;
    }
}
...
fn spawn_one(commands: &mut Commands, ev: &SpawnNpc, asset: &WzNpcAsset) {
    ...
    commands.spawn((
        ...
        Sprite {
            image: first_frame.sprite.image_handle.clone(),
            flip_x: ev.flip,
            ..default()
        },
        Transform::from_xyz(
            ev.x - first_frame.sprite.origin.x,
            ev.y - first_frame.sprite.origin.y,
